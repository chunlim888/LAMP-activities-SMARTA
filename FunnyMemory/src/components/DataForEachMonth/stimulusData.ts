...
const DEFAULT_IMAGE_BASE_URL =
  "https://raw.githubusercontent.com/chunlim888/LAMP-activities-SMARTA/refs/heads/funny-memory-new-images/FunnyImages-v2";
...
export const createDataForMonth = (month: number) => {
  ...
  if (!stimuli) {
    throw new Error(`No FunnyMemory stimulus data configured for month ${month}`);
  }

  return stimuli.map((stimulus, index) => {
    const trial = index + 1;
    const code = `M${pad(month)}_T${pad(trial)}`;
    const trialBase = `${IMAGE_BASE_URL}/month-${pad(month)}/trial-${pad(trial)}/${code}`;
